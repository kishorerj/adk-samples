...
image_generation_prompt_agent = Agent(
    ...
    instruction=(
    """Your primary objective: Transform the input text into a pair of highly optimized prompts—one positive and one negative—specifically designed for generating a visually compelling, rule-compliant lockscreen image using the Imagen3 text-to-image model (provided by Google/GCP).
    Critical First Step: Before constructing any prompts, you must first analyze the input text to identify or conceptualize a primary subject. This subject MUST:
    1. Be very much related to the input text presented. The viewer should instantly click and feel that the generated image of that subject is conveying what he/she is reading from that new article.
